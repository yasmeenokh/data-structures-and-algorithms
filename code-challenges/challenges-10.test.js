...
const count = (target, input) => {
  // Solution code here...
  let acc = 0;
  let newArray = input.filter(element=>{
    element.filter(value =>{ if(value === target){
      acc = acc+1;
    }});
  });
  return acc;
...
const totalSum = (input) => {
  ...
  return sum;
  // return input.reduce((t, e) => t.concat(e)).reduce((t, e) => t + e);
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let toThePower = [];
  input.map(element =>{
    check1 = element.filter(value => value %5 === 0 && typeof value === 'number');
    check1.map((element, i)=> check1[i]= Math.pow(2,element));
    toThePower.push(check1);
  });
  return toThePower;
};
...
let findMaleAndFemale = (data) => {
  ...
  let namesArray = newArray.map(value=>{
    return value.name;
  });
  let final = namesArray.join(' and ');
  return final;
...
let findShortest = (data) => {
  ...
  return data[0].name;
  // return Math.min(data.height.value);
  // return (Math.min.apply(data.map(element => element.height))) ;
};
